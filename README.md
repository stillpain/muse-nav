...
- 访问统计：首页展示累计/今日访客、页面访问量与国家地区分布；不使用 Cookie，也不保存原始 IP。
...
analytics_*  SQLite 内的访客摘要与每日聚合表（自动创建）
```

## 地理位置统计

地理分布默认读取 Cloudflare 免费提供的 `CF-IPCountry` 请求头。域名接入并开启 Cloudflare 代理后，在 Cloudflare 控制台的“网络”中启用 **IP 地理位置**；没有该请求头时访客仍会正常计数，地区显示为“待识别”。统计模块会忽略后台、账号页、静态资源、常见爬虫以及启用 DNT 的请求。

如果上游代理提供其他国家代码请求头，可在应用容器中设置 `MUSE_COUNTRY_HEADER`，值为该请求头名称。国家代码必须为 ISO 3166-1 两位大写代码。
